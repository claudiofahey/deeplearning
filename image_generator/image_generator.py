...
def process_image(input_filename, options):
    ...
    # Read input file.
    orgimg = cv2.imread(input_filename)
    if orgimg is None:
        raise Exception('Unable to read file %s' % input_filename)

    for i in range(options.output_images_per_input_image):
...
def main():
    ...
    parser.add_argument(
        '--output_images_per_input_image',
        type=int,
        default=1,
        help='Number of images to generate for each input image.'
    )
